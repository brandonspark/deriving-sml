structure Test :
  ...
  struct
    ...
    fun blue s = text TC.lightblue s

    fun suite_name >::: test_results =
      List.map
        (fn (test_name, results) =>
          case results of
            [] =>
              surround (text TC.green "Tests passed!: " ^ test_name ^ "\n")
          | _ =>
              List.map
                (fn (idx, expected, got) =>
                  text TC.red "Failure " ^ "at " ^ blue suite_name ^ ":" ^ blue
                  test_name ^ ":" ^ blue (Int.toString idx) ^ ":\n"
                  ^ "Expected: " ^ blue expected ^ " but got: " ^ blue got ^ "\n"
                )
                results
              |> String.concatWith "\n"
              |> surround
    ...
    fun process_tests toString l =
      List.mapPartiali
        (fn (idx, (s, s')) =>
          if s = s' then
            NONE
          else
            SOME (idx, toString s', toString s)
        )
        l
    ...
    fun run_map_tests l =
      ...
      |> process_tests [.show: bool]

    fun run_tests () = "deriving tests" >:::
      [ "show_tests" >:: run_show_tests ()
      , "eq_tests" >:: run_eq_tests ()
      , "cmp_tests" >:: run_cmp_tests ()
